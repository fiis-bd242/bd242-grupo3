...
```sql
    "Limit  (cost=45.43..45.43 rows=1 width=326) (actual time=0.112..0.113 rows=1 loops=1)"
"  ->  Sort  (cost=45.43..45.43 rows=1 width=326) (actual time=0.111..0.111 rows=1 loops=1)"
"        Sort Key: p.id_plan"
"        Sort Method: quicksort  Memory: 25kB"
"        ->  Nested Loop  (cost=0.73..45.42 rows=1 width=326) (actual time=0.077..0.103 rows=1 loops=1)"
"              ->  Nested Loop  (cost=0.58..45.18 rows=1 width=152) (actual time=0.066..0.092 rows=1 loops=1)"
"                    ->  Nested Loop  (cost=0.43..36.99 rows=1 width=78) (actual time=0.060..0.086 rows=1 loops=1)"
"                          ->  Nested Loop  (cost=0.28..28.81 rows=1 width=24) (actual time=0.055..0.080 rows=1 loops=1)"
"                                ->  Seq Scan on mantenimiento m  (cost=0.00..20.50 rows=1 width=20) (actual time=0.047..0.072 rows=1 loops=1)"
"                                      Filter: (fecha_inicio_programado = '2023-08-15'::date)"
"                                      Rows Removed by Filter: 999"
"                                ->  Index Scan using plan_de_mantenimiento_pkey on plan_de_mantenimiento p  (cost=0.28..8.29 rows=1 width=8) (actual time=0.006..0.006 rows=1 loops=1)"
"                                      Index Cond: (id_plan = m.id_plan)"
...
"                          Index Cond: (id_tipo_mant = m.id_tipo_mant)"
"              ->  Index Scan using criticidad_pkey on criticidad c  (cost=0.15..0.22 rows=1 width=122) (actual time=0.006..0.006 rows=1 loops=1)"
"                    Index Cond: (id_criticidad = p.id_criticidad)"
...
```sql
    "Limit  (cost=33.22..33.22 rows=1 width=326) (actual time=0.069..0.070 rows=1 loops=1)"
"  ->  Sort  (cost=33.22..33.22 rows=1 width=326) (actual time=0.068..0.069 rows=1 loops=1)"
"        Sort Key: p.id_plan"
"        Sort Method: quicksort  Memory: 25kB"
"        ->  Nested Loop  (cost=1.00..33.21 rows=1 width=326) (actual time=0.058..0.060 rows=1 loops=1)"
"              ->  Nested Loop  (cost=0.85..32.98 rows=1 width=152) (actual time=0.048..0.049 rows=1 loops=1)"
"                    ->  Nested Loop  (cost=0.70..24.79 rows=1 width=78) (actual time=0.041..0.043 rows=1 loops=1)"
"                          ->  Nested Loop  (cost=0.55..16.60 rows=1 width=24) (actual time=0.033..0.034 rows=1 loops=1)"
"                                ->  Index Scan using idx_fecha_program on mantenimiento m  (cost=0.28..8.29 rows=1 width=20) (actual time=0.025..0.025 rows=1 loops=1)"
...
"                                Index Cond: (id_estado = m.id_estado)"
"                    ->  Index Scan using tipo_mantenimiento_pkey on tipo_mantenimiento tm  (cost=0.15..8.17 rows=1 width=82) (actual time=0.006..0.006 rows=1 loops=1)"
"                          Index Cond: (id_tipo_mant = m.id_tipo_mant)"
...
* Comparación de tiempos de ejecución
	...
	- Sin índice, el tiempo de ejecución fue de 0.153 ms.
	- Con el índice, el tiempo de ejecución fue de 0.130 ms.
	- Esto muestra una mejora del 15.03% en el tiempo de ejecución.

* Comparación de costos

	- Sin índice, el costo fue de 45.43.
	- Con el índice, el costo fue de 33.22.
	- Esto muestra una reducción del 26.87% en el costo.
