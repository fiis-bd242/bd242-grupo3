...
```sql
    "Limit  (cost=86.20..87.75 rows=9 width=326) (actual time=0.720..0.739 rows=9 loops=1)"
    "  ->  Nested Loop  (cost=68.58..241.26 rows=1000 width=326) (actual time=0.490..0.734 rows=111 loops=1)"
    "         ->  Nested Loop  (cost=68.42..197.93 rows=1000 width=152) (actual time=0.478..0.640 rows=111 loops=1)"
    "               ->  Nested Loop  (cost=68.26..172.31 rows=1000 width=78) (actual time=0.470..0.591 rows=111 loops=1)"
    "                    ->  Merge Join  (cost=68.10..145.60 rows=1000 width=24) (actual time=0.456..0.525 rows=111 loops=1)"
    "              ->  Nested Loop  (cost=0.44..61.41 rows=3 width=94) (actual time=0.043..0.147 rows=3 loops=1)"
    "                          Merge Cond: (p.id_plan = m.id_plan)"
    "                          ->  Index Scan using plan_de_mantenimiento_pkey on plan_de_mantenimiento p  (cost=0.28..60.27 rows=1000 width=8) (actual time=0.009..0.032 rows=111 loops=1)"
    "                          ->  Sort  (cost=67.83..70.33 rows=1000 width=20) (actual time=0.444..0.449 rows=111 loops=1)"
    "                                Sort Method: quicksort  Memory: 64kB"
    "                          ->  Index Scan using plan_de_mantenimiento_pkey on plan_de_mantenimiento p  (cost=0.28..8.29 rows=1 width=8) (actual time=0.004..0.004 rows=1 loops=3)"
    "                                ->  Seq Scan on mantenimiento m  (cost=0.00..18.00 rows=1000 width=20) (actual time=0.013..0.187 rows=1000 loops=1)"
    "                    ->  Memoize  (cost=0.16..0.23 rows=1 width=62) (actual time=0.000..0.000 rows=1 loops=111)"
    "                          Cache Key: m.id_estado"
    "                          Cache Mode: logical"
    "                          ->  Index Scan using estado_mantto_pkey on estado_mantto em  (cost=0.15..0.22 rows=1 width=62) (actual time=0.002..0.002 rows=1 loops=7)"
    "                                Index Cond: (id_estado = m.id_estado))"
    "              ->  Memoize  (cost=0.16..0.23 rows=1 width=82) (actual time=0.000..0.000 rows=1 loops=111)"
    "              ->  Index Scan using criticidad_pkey on criticidad c  (cost=0.15..0.22 rows=1 width=122) (actual time=0.003..0.003 rows=1 loops=3)"
    "                    Cache Key: m.id_tipo_mant"
    "                    Cache Mode: logical"
    "                    Hits: 109  Misses: 2  Evictions: 0  Overflows: 0  Memory Usage: 1kB"
    ...
    "                          Index Cond: (id_tipo_mant = m.id_tipo_mant)"
    "        ->  Memoize  (cost=0.16..0.23 rows=1 width=122) (actual time=0.000..0.000 rows=1 loops=111)"
    "              Cache Key: p.id_criticidad"
    "              Cache Mode: logical"
    "              Hits: 108  Misses: 3  Evictions: 0  Overflows: 0  Memory Usage: 1kB"
    "              ->  Index Scan using criticidad_pkey on criticidad c  (cost=0.15..0.22 rows=1 width=122) (actual time=0.003..0.003 rows=1 loops=3)"
    "                    Index Cond: (id_criticidad = p.id_criticidad)"
...
```sql
    CREATE INDEX IDX_Fecha_programada
    ON Mantenimiento(fecha_inicio_programado)
```
...
```sql
    "Limit  (cost=86.20..87.75 rows=9 width=326) (actual time=0.421..0.433 rows=9 loops=1)"
    "  ->  Nested Loop  (cost=68.58..241.26 rows=1000 width=326) (actual time=0.274..0.429 rows=111 loops=1)"
    "        ->  Nested Loop  (cost=68.42..197.93 rows=1000 width=152) (actual time=0.264..0.368 rows=111 loops=1)"
    "              ->  Nested Loop  (cost=68.26..172.31 rows=1000 width=78) (actual time=0.255..0.333 rows=111 loops=1)"
    "                    ->  Merge Join  (cost=68.10..145.60 rows=1000 width=24) (actual time=0.248..0.290 rows=111 loops=1)"
    "                          Merge Cond: (p.id_plan = m.id_plan)"
    "                          ->  Index Scan using plan_de_mantenimiento_pkey on plan_de_mantenimiento p  (cost=0.28..60.27 rows=1000 width=8) (actual time=0.007..0.019 rows=111 loops=1)"
    "                          ->  Sort  (cost=67.83..70.33 rows=1000 width=20) (actual time=0.238..0.242 rows=111 loops=1)"
    "                                Sort Key: m.id_plan"
    "                                Sort Method: quicksort  Memory: 64kB"
    "                                ->  Seq Scan on mantenimiento m  (cost=0.00..18.00 rows=1000 width=20) (actual time=0.009..0.090 rows=1000 loops=1))"
    "                    ->  Memoize  (cost=0.16..0.23 rows=1 width=62) (actual time=0.000..0.000 rows=1 loops=111)"
    "                          Cache Key: m.id_estado"
    "                          Cache Mode: logical"
    "                          Hits: 104  Misses: 7  Evictions: 0  Overflows: 0  Memory Usage: 1kB"
    "                          ->  Index Scan using estado_mantto_pkey on estado_mantto em  (cost=0.15..0.22 rows=1 width=62) (actual time=0.001..0.001 rows=1 loops=7)"
    "                                Index Cond: (id_estado = m.id_estado)"
    "              ->  Memoize  (cost=0.16..0.23 rows=1 width=82) (actual time=0.000..0.000 rows=1 loops=111)"
    "                    Cache Key: m.id_tipo_mant"
    "                    Cache Mode: logical"
    ...
    "              Cache Mode: logical"
    "              Hits: 108  Misses: 3  Evictions: 0  Overflows: 0  Memory Usage: 1kB"
    "              ->  Index Scan using criticidad_pkey on criticidad c  (cost=0.15..0.22 rows=1 width=122) (actual time=0.002..0.002 rows=1 loops=3)"
    "                    Index Cond: (id_criticidad = p.id_criticidad)"
...
* Comparación de tiempos de ejecución
	...
	- Sin índice, el tiempo de ejecución fue de 0.803 ms.
	- Con el índice, el tiempo de ejecución fue de 0.492 ms.
	- Esto muestra una mejora del 38.7% en el tiempo de ejecución.

* Comparación de costos

	- Sin índice, el costo fue de 87.75.
	- Con el índice, el costo fue de 87.75.
