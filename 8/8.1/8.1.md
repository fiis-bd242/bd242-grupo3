...
3. Boton Guardar y Programar:
	...
	VALUES (<6>,DATE('<7>'),DATE('<8>'),null,<1>,<9>,<10>,<11>);

	--- Actualización en las tablas de recursos: Insumos y equipos de soporte:
	--- Esta implementación debe realizarse con una estructura repetitiva en el backend ---
	INSERT INTO EquipoSXMantenimiento (id_equipo_mant, id_act_mantto, id_equipo_soporte)
	VALUES (<12>, <6>, <13>);

	INSERT INTO InsumoXMantenimiento (id_insum_mant, cantidad, id_act_mantto, id_insumo)
	VALUES (<14>, <15>, <6>, <16>);
	```
	...
	| `<3>`       | Fecha de creación del plan, se obtiene automáticamente, formato: YYYY-MM-DD|
	...
	| `<7>`       | Fecha inicial del mantenimiento obtenida del formulario, formato: YYYY-MM-DD |
	| `<8>`       | Fecha final del mantenimiento obtenida del formulario, formato: YYYY-MM-DD |
	| `<9>`       | Id del tipo de mantenimiento obtenido del formulario |
	| `<10>`      | Id de la máquina obtenido del formulario |
	| `<11>`      | Id del estado luego de crear el plan. En este evento, será igual a 1 (Borrador) |
	| `<12>`       | Id de la tabla EquipoSoporteXMantenimiento creado automáticamente |
	| `<13>`       | Id de un elemento de la lista de equipos de soporte a insertar, obtenida del formulario |
	| `<14>`       | Id de la tabla InsumoXMantenimiento creado automáticamente |
	| `<15>`       | Cantidad de un elemento de la lista de insumos a insertar, obtenida del formulario |
	| `<16>`       | Id de un elemento de la lista de insumos a insertar, obtenida del formulario |
...
1. Lista de Planes de Mantenimiento:
	...
	SELECT LPAD(p.id_plan::TEXT, 4, '0'), CONCAT('MQ-',LPAD(m.id_maquina::TEXT, 4, '0')), tm.nombre_tipo_mant, c.nivel, m.fecha_inicio_programado
...
2. Botón Guardar y Programar:
	...
	INSERT INTO Orden_de_trabajo (id_orden,descripcion,fecha_orden, visible)
	VALUES (<1>,<2>,DATE('<3>'), 'Y');
	---------------------------------------------------------------------------
	...
	INSERT INTO Actividad_empleado (id_actvempleado, nombre_actv, descripcion_actv, fecha_inicio, fecha_fin, id_equipo_soporte, id_empleado, id_estado, id_orden)
	SELECT
    <5>,
    'Responsable',
	...
	--- Añadiendo el resto de empleados sin actividad asignada --- 
	--- Esta implementación debe realizarse con una estructura repetitiva en el backend ---
	INSERT INTO Actividad_empleado (id_actvempleado, nombre_actv, descripcion_actv, fecha_inicio, fecha_fin, id_equipo_soporte, id_empleado, id_estado, id_orden)
	VALUES (<5>, null, null, null, null, null, <7>, 1, <1>);
	...
	| `<6>`       | Id de un elemento empleado obtenido de la lista en el formulario |
...
1. Lista de Órdenes de Mantenimiento:
	...
	SELECT LPAD(p.id_plan::TEXT, 4, '0'), CONCAT('MQ-',LPAD(m.id_maquina::TEXT, 4, '0')), tm.nombre_tipo_mant, m.fecha_inicio_programado
	FROM Plan_de_Mantenimiento p 
	INNER JOIN Mantenimiento m
	ON m.id_plan = p.id_plan
	INNER JOIN Tipo_mantenimiento tm
	...
	|  | Descripción                                           |
	|-------------|-------------------------------------------------------|
	| `<1>`       | Número del id de la orden para empezar a mostrar, se obtiene del servidor |
...
1. Selección de orden de trabajo
	...
	```sql
	--- Obtiene la lista de ordenes de trabajo visibles ---
	SELECT CONCAT('OT-', LPAD(id_orden::TEXT, 4, '0'))
...
2. Lista de Actividades de Empleados:
	...
	```sql
	--- Obtiene el resumen de las actividades de empleado ---
	SELECT LPAD(a.id_actvempleado::TEXT, 4, '0'), emp.nombre, CONCAT('ES-',LPAD(es.id_equipo_soporte::TEXT, 4, '0')), a.fecha_inicio
	...
	| `<2>`       | Id de la actividad de mantenimiento para empezar a mostrar |
...
1. Detalle completo del plan de mantenimiento
	...
	SELECT CONCAT('PL-', LPAD(p.id_plan::TEXT, 4, '0')), 
       tm.nombre_tipo_mant, 
...
2. Notificación del responsable (funcionalidad principal):
	...
	INSERT INTO Notificaciones (id_notificacion, asunto, mensaje, fecha_notificacion, id_remitente, id_destinatario, id_registro, id_reporte, id_tipo)
	SELECT <2>,'Nuevo plan por aceptar',CONCAT('Nuevo plan de mantenimiento disponible con fecha ' , '<3>'), DATE('<3>'), 1, e.id_empleado, null, null, 2
	FROM Empleado e
		...
		WHERE p.id_plan = 1001 AND act.nombre_actv = 'Responsable';

	--- Se cambia el estado del mantenimiento a notificado --- 
	UPDATE Mantenimiento
	SET id_estado = 2
	WHERE id_plan = <1>;
	```
...
1. Detalle completo del plan de mantenimiento
	...
	SELECT CONCAT('PL-', LPAD(p.id_plan::TEXT, 4, '0')), CONCAT('OT-', LPAD(o.id_orden::TEXT, 4, '0')), tm.nombre_tipo_mant, crit.nivel, CONCAT('MQ-', LPAD(m.id_maquina::TEXT, 4, '0')), m.fecha_inicio_programado, m.fecha_fin_programado, p.descripcion,
	...
	WHERE act.nombre_actv = 'Responsable' AND p.id_plan = <1>;
...
2. Botón Guardar:
	...
	--- Esta implementación debe realizarse con una estructura repetitiva en el backend ---
	DELETE FROM EquipoSXMantenimiento
	...
	INSERT INTO EquipoSXMantenimiento (id_equipo_mant, id_act_mantto, id_equipo_soporte)
	SELECT 
    <9>, m.id_act_mantto, <10>
	FROM Mantenimiento m WHERE m.id_plan = <1>;
...
3. Borrar Plan (esto borra el plan y su orden asociada)
...
1. Detalle de la orden (para autocompletar los campos)
    ...
    -- Lista de empleados en formato "id_empleado-nombre"
...
1. Detalle de la orden (para autocompletar los campos)
	...
	--- Obtiene el detalle de la orden de trabajo ---
	--- Se acceden a los datos de las tablas plan de mantenimiento, mantenimiento,  actividad empleado y orden de trabajo ---
	SELECT CONCAT('OT-', LPAD(o.id_orden::TEXT, 4, '0')), CONCAT('PL-', LPAD(p.id_plan::TEXT, 4, '0')), o.descripcion,
	...
	WHERE o.id_orden = <1>
	GROUP BY 
		o.id_orden, p.id_plan, o.descripcion
	ORDER BY o.id_orden;
...
2. Botón Guardar:
	...
	| `<5>`       | Id del plan obtenido del servidor |

3. Botón Eliminar:
   
   ```sql
	--- Actualiza la visibilidad de la orden (no se elimina como tal) ---
	UPDATE Orden_de_trabajo
	SET visible = 'N'
	WHERE id_orden = <1>
	```
...
1. Detalle completo de la actividad del empleado
	...
	SELECT CONCAT('AC-', LPAD(act.id_actvempleado::TEXT, 4, '0')), act.id_estado, CONCAT('MQ-', LPAD(m.id_maquina::TEXT, 4, '0')), act.nombre_actv, emp.id_empleado, emp.nombre, act.fecha_inicio, act.fecha_fin, es.id_equipo_soporte, es.nombre_equipo_soporte, o.id_orden, act.descripcion_actv
...
1. Detalle de la actividad (para autocompletar los campos)
	...
	SELECT act.id_actvempleado, act.nombre_actv, emp.id_empleado, emp.nombre, act.fecha_inicio, act.fecha_fin, es.id_equipo_soporte, es.nombre_equipo_soporte, act.descripcion_actv
...
2. Botón Guardar:
	...
	--- Actualiza las tablas de plan y mantenimiento ---
	UPDATE Actividad_Empleado
	SET nombre_actv = <2>, id_empleado = <3>, id_equipo_soporte = <4> fecha_inicio = <5>, fecha_fin = <6>, descripcion_actv = <7>
...
3. Confirmar reserva de pedidos: Se da click en este botón para confirmar la reserva con todos los insumos solicitados; en este momento se procede a ejecutar una serie de sentencias sql.
	...
	---Transferencia de data de la tabla temporal
...
2. Ver fecha: Se da click en este botón para observar el detalle de la fecha de registro de la reserva.
...
3. Ver insumos solicitados por reserva: Se da click en este botón para observar el detalle de todos los insumos que fueron solicitados en la reserva.
...
4. Ver estado de reserva: Esta consulta sirve para saber que boton resaltar con el color morado en la sección estado.
...
4. Cambiar el botón de estado: Se da click en cuaqluiera de los botones para cambiar el estado de las solicitudes.
...
![Equipo de Soporte](../imagenes/Dashboard-módulo-equipos-soporte.png)
...
![Equipo](../imagenes/Inventario-equipos-de-soporte.png)
...
2. Boton ver para ver con mas detalle las condiciones del equipo de soporte en especifico.
	...
	SELECT
		es.id_equipo_soporte AS ID,
		es.nombre_equipo_soporte AS Nombre,
		es.fecha_adquisicion AS "Fecha de Adquisicion",
		tes.nombre_tipo AS Tipo,
		ees.nombre_estado AS Estado,
		des.nombre_disponibilidad AS Disponibilidad
		CASE 
			WHEN id_act_mantto is not NULL THEN id_act_mantto
			ELSE NULL
		END AS "Mantenimiento Asignado"
	FROM equipo_de_soporte es
	INNER JOIN tipo_equipo_soporte tes
	ON es.id_tipo = tes.id_tipo
	INNER JOIN estado_equipo_soporte ees
...
3. Boton buscar para filtrar algun equipo de soporte en especifico que se requiera ver en base a un procesamiento almacenado
...
![Equipo de Soporte](../imagenes/Solicitud-pedido-compra-equipos-de-soporte.png)
...
1. Boton para añadir los equipos de soporte a la solicitud de pedido de compra, mostrando una pantalla para su seleccion el cual puede ser mas de uno.
	Primero encontramos todos lo equipos de soporte para seleccionar y escogemos
	```sql
	SELECT
		id_equipo_soporte AS ID,
		nombre_equipo_soporte AS Nombre
	FROM Equipo_de_soporte
	```
2. Rellenar los campos requeridos para crear el pedido de compra
	```sql
	INSERT INTO Pedido_compra (fecha_pedido_compra, hora_pedido_compra, descripcion, id_urgencia, id_estado_pedido, id_empleado)
	VALUES
	(CURRENT_DATE, CURRENT_TIME, <3>, <1>, 0, <2>)
	```
	...
	```sql
	INSERT INTO Detalle_Pedido_compra (cantidad, id_producto, precio_unitario, id_pedido_compra, id_tipo_producto)
	VALUES
	(<1>,<2>,<3>,<4>, 2)
	...
	| Marcador | Descripción                                           |
	|-------------|-------------------------------------------------------|
	| `<1>`       | Cantidad del equipo           |
	| `<2>`       | id del equipo |
	| `<3>`       | precio unitario del equipo      |
	| `<4>`       | id del pedido de compra relacionado      |
...
![Equipo de Soporte](../imagenes/Gestion-pedidos-compra-de-equipos-soporte.png)
...
1. Visualizar el id, urgencia, fecha y estado de los pedidos solicitados de los equipos de soporte
	```sql
		...
		pc.fecha_pedido_compra AS "Fecha del pedido",
...
2. Boton ver para poder apreciar con mas detalle el pedido que se realizo
	```sql
		SELECT
		tp.nombre_producto AS "Tipo de Producto",
		dpc.id_producto AS "Productos",
		pc.descripción AS "Descripcion"
	FROM Detalle_pedido_compra dpc
	INNER JOIN equipo_de_soporte es
	ON dpc.id_producto = es.id_equipo_soporte
	INNER JOIN tipo_producto tp
	ON dpc.id_tipo_producto = tp.id_tipo_producto
	INNER JOIN pedido_compra pc
	ON dpc.id_pedido_compra = pc.id_pedido_compra
	WHERE dpc.id_pedido_compra = <1>
	```
	
	| Marcador | Descripción                                           |
	|-------------|-------------------------------------------------------|
	| `<1>`       | Id del pedido de compra especifico   |
...
1. **Rellenar Desplegable:** Se busca rellenar el desplegable con datos apropiados o que cumplan con los filtros.
	...
	ON man.Id_maquina = maq.id_maquina
	INNER JOIN Marca_maquina AS mm
	ON maq.id_marca_maquina = mm.id_marca_maquina
	WHERE 
		(Fecha_inicio_programado = <2> OR <2> IS NULL) 
		AND (nombre_marca LIKE <1> OR <1> IS NULL);
