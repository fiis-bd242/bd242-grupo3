...
1. Lista de Órdenes de Mantenimiento:
	...
	SELECT p.id_plan, m.id_maquina, tm.nombre_tipo_mant, m.fecha_inicio_programado
	FROM Plan_de_Mantenimiento p 
	INNER JOIN Mantenimiento m
	ON m.id_plan = p.id_plan
	INNER JOIN Tipo_mantenimiento tm
...
2. Notificación del responsable (funcionalidad principal):
	...
	SELECT 501,'Nuevo plan por aceptar',CONCAT('Nuevo plan de mantenimiento disponible con fecha ' , '2024-11-13'), DATE('2024-11-13'), 1, e.id_empleado, null, null, 2
	FROM Empleado e
...
2. Botón Guardar:
	...
	WHERE a.nombre_actv IN (
		...
		WHERE p.id_plan = 1001 AND a.nombre_actv = 'Responsable'
	);
...
3. Borrar Plan (esto borra el plan y su orden asociada)
	...
	WHERE id_orden IN (
		SELECT o.id_orden FROM Orden_de_trabajo o
		INNER JOIN Mantenimiento m ON m.id_orden = o.id_orden
		WHERE m.id_plan = 1001
	);

	DELETE FROM Plan_de_Mantenimiento p
	WHERE p.id_plan = 1001;

	DELETE FROM Mantenimiento m
	WHERE m.id_plan = 1001;
